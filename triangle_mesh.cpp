...
void TriangleMesh::write_stl(std::ofstream& out,const std::string& mesh_name) const
{
  progress_start(100,"Generating STL");

  QByteArray STLdata;
  QString header("Generated by Fracplanet. \n\n");
  header.fill('\0', 80-header.size());

  char trash[2] = {'\0', '\0'};
  uint ntriangles = triangles();

  STLdata.append(header.toLatin1(), 80);
  STLdata.append((char*) &ntriangles, sizeof(unsigned));
  ...
  for (uint i=0;i<triangles();i++)
    {
      ...
      const XYZ _triangle_normal = triangle_normal(i);

      STLdata.append((char*)&_triangle_normal.x, sizeof(float));
      STLdata.append((char*)&_triangle_normal.y, sizeof(float));
      STLdata.append((char*)&_triangle_normal.z, sizeof(float));

      STLdata.append((char*)&vertex_positions[0].x, sizeof(float));
      STLdata.append((char*)&vertex_positions[0].y, sizeof(float));
      STLdata.append((char*)&vertex_positions[0].z, sizeof(float));

      STLdata.append((char*)&vertex_positions[1].x, sizeof(float));
      STLdata.append((char*)&vertex_positions[1].y, sizeof(float));
      STLdata.append((char*)&vertex_positions[1].z, sizeof(float));

      STLdata.append((char*)&vertex_positions[2].x, sizeof(float));
      STLdata.append((char*)&vertex_positions[2].y, sizeof(float));
      STLdata.append((char*)&vertex_positions[2].z, sizeof(float));

      STLdata.append(trash, 2);

      progress_step((100*i)/(triangles()-1));
    }


    out.write(STLdata.data(), STLdata.size());
